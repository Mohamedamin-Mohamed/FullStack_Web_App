 ...
 * Modified: 11/21/2023                    
 * Description: This file is the backbone of the whole game @function drawboard() renders the game grid by dynamically generating a table of input fields, 
 * @function hiddenfields() creates hidden form fields to store state like item positions, 
 * @function pickmessage() randomly picks a message string for a board item, ensuring uniqueness, 
 * @function additem(type) adds a random item of specified type to open board position. Handles item state.
 * @function move(direction) changes robot position, checks item encounters, and handles game end logic.
 * @function startgame() Initializes game state, board, and items, including robot. Resets on new game.
 * @function saveHighScore(initials, score) Asynchronously calls to server to save passed score data.
 */
...
//this variable will tally users score
let score = 0;

//stores users intials in each session
let initials = ''

/**
 * Array of messages associated with different items on the board
 */
...
/**
 * Array of symbols associated with each item on the board
 */
...
/**
 * Function to draw the game board dynamically
 */
function drawboard() {
  //write the cells
  for (i = 0; i < rownum; i++) {
...
/**
 * Function to create hidden fields for storing information about items on the board
 */
...
/**
 * Function to randomly pick a message for an item
 */
...
function move(direction) {
  //increment the current users score
  score++;
  ...
  if (document.getElementById('r' + rchange + 'c' + cchange).value == '') {
    ...
    //this condition statement is invoked as soon the user wins
    else {
      console.log(initials, score) //testing purpose

      //store the users intials and score at the specified endpoint
      saveHighScore(initials, score)

      //Delay and then display the table that contains the Users stats
      setTimeout(() => {
        alert('Game Over, Scores will be displayed!!!');
        renderTable(); // Call renderTable after the alert
      }, 4000);
...
/**
 * Starts a new game, initializing the game board and adding items, including the robot.
 * Prompts the user to enter their initials at the beginning of the game.
 * Resets the game board, clears hidden fields, adds items, and places the robot on the board.
 */
function startgame() {
  // Prompt the user to enter their initials
  initials = prompt('Enter your Initials');

  // Clear the main game board to start a new game
  document.forms.rfk.reset();

  // Clear hidden fields used to store item information
  document.forms.hidden.reset();

  // Add random items to the game board
  for (i = 0; i < itemnum; i++) {
    additem();
  }

  // Add the robot to the game board
  additem('robot');
}
